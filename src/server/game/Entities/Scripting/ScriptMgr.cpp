...
void ScriptMgr::Initialize()
{
    ...
    std::unordered_set<std::string> dbReferencedScriptNames = sObjectMgr->GetAllDBScriptNames();
    ...
    std::unordered_set<std::string> coreOnlyScriptNames = sObjectMgr->GetAllDBScriptNames();
    for (std::string const& scriptName : dbReferencedScriptNames)
        coreOnlyScriptNames.erase(scriptName);

    coreOnlyScriptNames.erase("");

    for (std::string const& scriptName : coreOnlyScriptNames)
        TC_LOG_ERROR("sql.sql", "Script '{}' exists in the core but is not assigned in the database!", scriptName);
...
{
    if (QuestScript* questScript = ScriptRegistry<QuestScript>::Instance()->GetScriptById(quest->GetScriptId()))
        questScript->OnQuestAbandon(player, quest);
